...
export interface VMCore {
  ...
  /**
   * Parse + run Ruby source on the live VM. Returns 0 on success, 1 on
   * parse/runtime error. Throws `NotImplementedError` in compiler-less
   * builds (use {@link loadBytecode} instead).
   */
  eval(source: string): number;

  /**
   * Load pre-compiled mruby bytecode (mrbc output). Returns 0 on
   * success, 1 on runtime error. Accepts `Uint8Array` or `ArrayBuffer`.
   * Available in every build variant.
   */
  loadBytecode(bytes: Uint8Array | ArrayBuffer): number;
   ...
   */
  evalScript(selector: string): number;
