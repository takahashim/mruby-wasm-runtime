Spec.describe "MRubyWasm::Widget mount + refs + events" do
  Spec.assert "Counter mounts and click updates count" do
    ...
    counter_class = Class.new(MRubyWasm::Widget) do
      def setup
        @count = signal(0)
    ...
    MRubyWasm.register_widget "counter", counter_class
    MRubyWasm.start
  ...
  Spec.assert "missing ref raises with widget name and ref name" do
    ...
    MRubyWasm.register_widget "missing-ref-test", klass
    MRubyWasm.start
  ...
  Spec.assert "cleanup runs on element removal" do
    ...
    MRubyWasm.register_widget "cleanup-test", klass
    MRubyWasm.start
