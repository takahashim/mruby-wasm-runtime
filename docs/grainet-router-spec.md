...
- 既存の `signal` / `memo` / `effect` / `bind` だけで route 切替 UI が書ける

中心方針:

- **location は signal**、**path helpers / named match は memo**、**navigate で書き込み**
...
- **nested declarative routes**: `<Routes><Route path="/a"><Route path="b"/></Route></Routes>` のような階層的 declaration は提供しない (memo 合成で書ける)
...
class HomePage < Grainet::Widget
  def setup
    bind refs.title, text: memo { "path=#{router.path}" }
  end
end
```
...
| 層 | 用途 | 主要 API |
|---|---|---|
| 低レベル | URL 状態を signal/memo として扱うだけ。表示は呼び出し側で書く | `location` / `path` / `query` / `match` / `navigate` / `intercept_link` |
...
- **named match memo**: `router.home_match` / `user_match` (paramater 抽出済み Hash か nil)
- **active route management**: `router.current` (memo: `:home` / `:user` / `nil`)
...
class HomePage < Grainet::Widget
  def setup
    bind root, hidden: memo { router.path != "/" }
  end
end
...
class UserDetail < Grainet::Widget
  def setup
    ...
    bind root, hidden: memo { m.value.nil? }
    bind refs.title, text: memo { m.value&.dig(:id) }
  end
end
...
### `router.user_match` → Memo

route のマッチ結果を memo として返す:
...
- 低レベル: pattern 文字列を毎回書く、widget ごとに同じパターンを呼ぶと memo が重複生成される (性能劣化)
...
class UserDetail < Grainet::Widget
  def setup
    bind refs.title, text: memo { router.params[:id] }
  end
end
```

reactive コンテキスト (`memo` / `effect` / `bind`) 内で呼ぶと、内部で `location` signal を読むため URL 変化に応じて自動再計算される。
...
```ruby
bind refs.home_link, class: { active: memo { router.current == :home } }
```
...
それぞれ `location.value[:path]` 等のショートカット。reactive コンテキスト (memo / effect / bind) で呼ぶと依存登録される。

### `router.match(pattern)` → Memo

任意の pattern に対する match memo。DSL `*_match` の generic 版。
...
DSL を使わない / 動的 pattern を作りたい場合に使う。同じ pattern を複数の widget で使う場合は memo が重複するので、widget の ivar / instance 共有を検討。
...
| `href:` | String / Symbol / Signal / Memo / Proc | route path。Signal / Memo / Proc は値が動的に変わる場合に使う |
...
class Nav < Grainet::Widget
  def setup
    bind refs.home_link,  class: { active: memo { router.current == :home } }
    bind refs.about_link, class: { active: memo { router.current == :about } }
    root.on(:click) { |e| router.intercept_link(e) }
  end
end
```
...
class UserDetail < Grainet::Widget
  def setup
    bind refs.title, text: memo { "User #{router.params[:id]}" }
  end
end
```
...
class SearchPage < Grainet::Widget
  def setup
    ...
    bind_list refs.results, memo { search_for(@query.value) }, key: "id" do |item, t|
      t.text item["name"]
    end
  end
end
...
- mruby-grainet (core): 変更なし。`Grainet::Router` は core の `signal` / `memo` / `effect` / `Grainet::Template` を消費するだけ
...
### なぜ `match` を memo として返す?

- signal の依存追跡で widget が自動再評価される
- 手動 listener / unsubscribe 不要
- bind / effect で消費するときの書き味が他の memo と完全に同じ
...
router.home_path                # => "/"
router.user_path(id: 42)        # => "/users/42"
router.user_match               # → Memo { id: "42" } | nil
...
router.match("/foo/:id")        # → Memo { id: "..." } | nil
router.navigate("/foo")
...
```ruby
# Active link
bind link, class: { active: memo { router.current == :home } }
...
`{ path:, query:, hash: }` を毎回新しい Hash で生成すると、ハッシュ全体が変わり path のみ依存している memo も全部再計算される。signal は前回値と `==` 比較で短絡する設計を確認 (mruby-grainet core の Signal 実装を要 audit)。必要なら `path` / `query` / `hash` 個別 signal を内部で持って `location` は memo として合成する。
...
def page(name, pattern, template: nil)
  ...
  define_singleton_method("#{name}_match") do
    @cached_matches[name] ||= memo { match_path(compiled, location.value[:path]) }
  end
end
