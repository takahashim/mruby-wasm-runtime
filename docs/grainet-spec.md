...
- 既知の制約: 行削除時、block 内で作った per-row `bind` の effect / computed は widget の lifecycle ストアに残り続ける (orphan)。orphan effect は依存 signal が変わらないので再走しないため機能影響なし、メモリも O(削除済み行数) で軽微。bind_list 単位の lifecycle 管理は将来の改善余地
